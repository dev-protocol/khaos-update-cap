...
import { getDevBalanceOfLiquidityPool, getWEthBalanceOfLiquidityPool } from './balance'
...
export const getCap = async (provider: providers.BaseProvider): Promise<BigNumber> => {
	...
	const authinticatedPropertyList = getAuthinticatedPropertyList(authinticatedPropertoes)
	const lockupValuesMap = getLockupValuesMap(lockupSumValues)
	const geometricMean = calculateGeometricMean(lockupValuesMap, authinticatedPropertyList)
	const arithmeticMean = calculateArithmeticMean(lockupValuesMap, authinticatedPropertyList)
